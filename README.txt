...
- **Smart Home Control**: Interfaces with Home Assistant for lights (on/off/color), thermostats (set/adjust), and RGB lighting (OpenRGB/Logitech).
- **Screencast Support**: Enables Miracast for casting to TVs with confirmation prompts.
- **Security Features**: PIN authentication for sensitive actions, config encryption with Fernet, subprocess whitelisting, and LLM response sanitization with bleach.
- **Production Hardening**: Auto-restart on memory leaks, structured JSON logging, and dependency validation.
- **Scalability Enhancements**: Caching with lru_cache, dynamic power management for CPU/GPU, and thread-safe state management.
...
## Installation & Setup
1. Clone the repository: `git clone https://github.com/DailyDriver007/RTX-SYLPH`
2. Install dependencies: `pip install -r requirements.txt`
3. Configure config.json with your API keys (GROK_API_KEY, OPENAI_API_KEY, NVIDIA_API_KEY, HUGGINGFACE_API_KEY) and preferences (e.g., WAKE_WORD, FPS). Encrypt the file using Fernet for security.
4. Place your animated video in assets/rtx_sylph_animated.mp4.
5. Run: `python sylph.py` (or the compiled .exe for Windows).
For executable builds, use PyInstaller:
`pyinstaller --onefile --windowed --noconfirm --name "RTX_Sylph_TITAN_v7.7_Final_Lockdown" --icon=assets/sylph.ico --add-data "assets;assets" --hidden-import=GPUtil --hidden-import=psutil --hidden-import=pyttsx3.drivers.sapi5 --add-data "config.json;." sylph.py`
...
Challenges with more recent Python releases (e.g., 3.14) were addressed through virtual environments to isolate the development process, allowing RTX Sylph to run seamlessly on current-generation systems without disrupting the core proprietary dependencies. Challenges like voice recognition accuracy in noisy environments were mitigated with ambient noise adjustment, and LLM query failures were handled with exponential backoff retries.
